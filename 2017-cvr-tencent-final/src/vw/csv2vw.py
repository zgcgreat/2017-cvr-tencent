...
def vw_train():
    with open(save_path + 'train.vw', 'w') as outfile:
        ...
        for t, row in enumerate(DictReader(open(data_path + 'traincp.csv', 'r')), start=1):
            # if 24 <= int(row['date']) <= 29:
...
def vw_valid():
    with open(save_path + 'valid.vw', 'w') as outfile, open(save_path + 'validation.csv', 'w') as f_va:
        f_va.write('id,label\n')
        for t, row in enumerate(DictReader(open(data_path + 'train.csv', 'r')), start=1):
            if int(row['date']) == 23:
