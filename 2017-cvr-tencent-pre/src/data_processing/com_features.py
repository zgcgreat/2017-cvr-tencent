...
def tr_feat(start_date, fo):
    ...
    for i, row in enumerate(csv.DictReader(open('../../data/train.csv')), start=1):
        ...
        if start_date == date:
            ...
            #          ','.join(clk_fectr_feats[k]ats[k] for k in clk_feats.keys()) + ',' +
            #          ','.join(str(cnv_feats[k]) for k in cnv_feats.keys()) + '\n')
...
def te_feat(fo):
    ...
    for i, row in enumerate(csv.DictReader(open('../../data/test.csv')), start=1):
        ...
        for k in com_feats.keys():
            ...
            # fo.write(row['label'] + ',' + ','.join(features[k] for k in features.keys()) + ',' +
            #          ','.join(clk_feats[k] for k in clk_feats.keys()) + ',' +
            #          ','.join(str(cnv_feats[k]) for k in cnv_feats.keys()) + '\n')
        outrow = fo_row(row['label'], features, clk_feats, cnv_feats)
        fo.write(outrow + '\n')
